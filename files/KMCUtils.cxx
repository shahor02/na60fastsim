...
void  MagField::SetMagVTParam(const std::string& paramString, const std::vector<float>& parVals, float zref, float maxX, float maxY)
{
  ...
  fXMax[0] = maxX;
  fYMax[0] = maxY;
  printf("Set parameterized field for VerTel dipole, reference Z = %f, |X|<%f |Y|<%f\n", fZRef[0], maxX, maxY);
  fFieldFunReg[0]->Print();
}

//__________________________________________________
void  MagField::SetMagMSParam(const std::string& paramString, const std::vector<float>& parVals, float zref, float maxX, float maxY)
{
  ...
  fXMax[1] = maxX;
  fYMax[1] = maxY;
  printf("Set parameterized field for MuSpec dipole, reference Z = %f, |X|<%f |Y|<%f\n", fZRef[1], maxX, maxY);  
...
void MagField::Field(const Double_t *xyz, Double_t *bxyz) 
{
  ...
  for (int ir=0;ir<kNReg;ir++) {
    if((xyz[2] >fZMin[ir]) && (xyz[2] < fZMax[ir])) {
      if (std::abs(xyz[0])>fXMax[ir] || std::abs(xyz[1])>fYMax[ir]) {
	return;
      }
      if (fFieldFunReg[ir]) {
