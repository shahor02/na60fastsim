  ...
  ,fTrack( src.fTrack )
{
  for (int i=kMaxITSLr;i--;) {
    fClID[i]=-src.fClID[i];
    fChi2CorrCl[i] = src.fChi2CorrCl[i];
    fChi2WinCl[i] = src.fChi2WinCl[i];
  }
}
...
Bool_t KMCProbeFwd::ApplyMSEL(double x2X0, double xTimesRho)
{
  ...
  if (xTimesRho<-1e-7) {
    ...
    Double_t bg = p/fMass;
    Double_t dEmean=AliExternalTrackParam::BetheBlochSolid(bg)*xTimesRho; // mean loss
...
void KMCProbeFwd::Print(Option_t* opt) const
{
  ...
  if (opts.Contains("clid")) {
    for (int ilr=0;ilr<fgNITSLayers;ilr++) {
      printf("%4d %f.2/%.2f|",fClID[ilr], fChi2CorrCl[ilr], fChi2WinCl[ilr]);
    }
  }
  printf("\n");
