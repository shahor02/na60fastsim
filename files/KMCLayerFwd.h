...
public:
  enum {kTypeNA=-1,kVTX,kITS,kMS,kTRIG,kABS,kDUMMY,kMAG, kNTypes, kBitVertex=BIT(15)};
  enum {kMaxAccReg = 5};
  ...
  KMCClusterFwd*        GetCluster(int i)        const {return i<0 ? GetMCCluster() : GetBgCluster(i); }
  KMCClusterFwd*        GetCluster(int i)              {return i<0 ? GetMCCluster() : GetBgCluster(i); }
  //
