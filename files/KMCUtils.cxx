...
void  MagField::SetMagVTParam(const std::string& paramString, const std::vector<float>& parVals)
{
  ...
  printf("Set parameterized field for VerTel dipole");
  fFieldFunReg[0]->Print();
}

//__________________________________________________
void  MagField::SetMagMSParam(const std::string& paramString, const std::vector<float>& parVals)
{
  fFieldFunReg[1] = new TF1("dipMS", paramString.c_str(), fZMin[1], fZMax[1]);
  for (int i=0;i<(int)parVals.size();i++) {
    fFieldFunReg[1]->SetParameter(i, parVals[i]);
  }
  printf("Set parameterized field for MuSpec dipole");
...
void MagField::Field(const Double_t *xyz, Double_t *bxyz) 
{
  ...
  for (int ir=0;ir<kNReg;ir++) {
    if((xyz[2] >fZMin[ir]) && (xyz[2] < fZMax[ir])) {
      if (fFieldFunReg[ir]) {
	bxyz[0] = fFieldFunReg[ir]->Eval(xyz[2] - (fZMin[ir]+fZMax[ir])/2.);
	return;
      }
      if(ir == 0) {
