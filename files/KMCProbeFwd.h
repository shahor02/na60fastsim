...
inline Bool_t KMCProbeFwd::PropagateToZBxByBz(double z, const double *bxyz)
{
  ...
  AliDebug(2,Form("To Z=%.3f, B: %.4f %.4f %.4f",z,bxyz[0],bxyz[1],bxyz[2])); 
  //  if (AliLog::GetGlobalDebugLevel()>2) {printf(">>");fTrack.Print();}
  Bool_t res = fTrack.PropagateToBxByBz(z,bxyz);
  //  if (AliLog::GetGlobalDebugLevel()>2 && !res) {printf("<<res%d<<",res);fTrack.Print();}
  return res;
  //
}
...
inline Bool_t KMCProbeFwd::CorrectForMeanMaterial(double xOverX0, double xTimesRho, Bool_t modeMC, Bool_t anglecorr)
{
  ...
  if (modeMC) {
    if (!(res=ApplyMSEL(xOverX0,xTimesRho))) {
      AliDebug(2,Form("Failed to apply MS, %f",xOverX0));
      if (AliLog::GetGlobalDebugLevel()>2) fTrack.Print();
      return kFALSE;
    }
  ...
  if (!res) {
    AliDebug(2,Form("Failed to CorrectForMeanMaterial: %f %f",xOverX0,xTimesRho));
    if (AliLog::GetGlobalDebugLevel()>2) fTrack.Print();
    return res;
  }
  return kTRUE;
